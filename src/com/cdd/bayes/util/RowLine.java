...
public class RowLine extends Pane
{
	...
	protected void layoutChildren()
	{
		...
		for (int n = 0; n < sz; n++)
		{
			...
			else if (u.valign == CENTRE) y = 0.5 * (height - h[n]);
			else if (u.valign == BASELINE)
			{
				double wb = u.widget.getBaselineOffset();
				if (Double.isFinite(wb)) y = baseline - wb; else y = 0.5 * (height - h[n]);
			}
